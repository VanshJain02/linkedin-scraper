...
# async def scrape_linkedin_jobs(query="software engineer", location="India", target_titles=None, role_type_filter=None, limit=10):
...
#             posted_el = await card.query_selector("time")
...
#             if not job_url or "authwall" in job_url:
#                 continue


#             posted = await posted_el.get_attribute("datetime") if posted_el else ""

#             repost_label = await card.query_selector(".result-benefits__text")
#             if repost_label:
#                 repost_text = await repost_label.inner_text()
#                 if "Reposted" in repost_text:
#                     continue

#             if await is_semantically_relevant(title, target_titles):
#                 job_page = await browser.new_page()
#                 await job_page.goto(job_url)
#                 await job_page.wait_for_selector(".show-more-less-html__markup", timeout=10000)
#                 desc_el = await job_page.query_selector(".show-more-less-html__markup")
